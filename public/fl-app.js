...
function stopTracking(){
  ...
  // ── Publish updated stats to localStorage for leaderboard ──────────────────
  setTimeout(function() {
    try {
      var uid = window.FL_USER && window.FL_USER.uid;
      if (!uid) return;
      var name = window.FL_USER.displayName || window.FL_USER.email || 'User';
      var initials = name.split(' ').map(function(w){return w[0]||'';}).join('').slice(0,2).toUpperCase() || '??';
      var credits = parseInt(localStorage.getItem('fl_credits')||'0',10);
      var sessions = [];
      try { sessions = JSON.parse(localStorage.getItem('fl_s_' + uid) || '[]'); } catch(e) {}

      var today = new Date().toLocaleDateString();
      var weekAgo = Date.now() - 7 * 24 * 3600 * 1000;
      var todayStr = new Date().toLocaleDateString(undefined, {day:'numeric',month:'short',year:'numeric'});

      var todaySessions = sessions.filter(function(s){return s.date === todayStr;});
      var weekSessions  = sessions.filter(function(s){return s.id > weekAgo;});

      var focusMinsToday   = Math.round(todaySessions.reduce(function(a,s){return a+(s.focusedSecs||0);},0) / 60);
      var focusMinsWeek    = Math.round(weekSessions.reduce(function(a,s){return a+(s.focusedSecs||0);},0) / 60);
      var focusMinsAllTime = Math.round(sessions.reduce(function(a,s){return a+(s.focusedSecs||0);},0) / 60);

      // Streak calculation
      var streak = 0;
      var daySet = {};
      sessions.forEach(function(s){ daySet[new Date(s.id).toLocaleDateString()] = true; });
      var d = new Date();
      while (daySet[d.toLocaleDateString()]) { streak++; d.setDate(d.getDate()-1); }

      var levelNames = ['Beginner','Focused','Pro','Expert','Master'];
      var levelThresholds = [0,50,150,350,700];
      var levelIdx = 0;
      for (var li = levelThresholds.length-1; li >= 0; li--) {
        if (credits >= levelThresholds[li]) { levelIdx = li; break; }
      }

      var statsObj = {
        uid: uid, name: name, avatar: initials,
        focusMinsToday: focusMinsToday,
        focusMinsWeek: focusMinsWeek,
        focusMinsAllTime: focusMinsAllTime,
        sessionsToday: todaySessions.length,
        sessionsTotal: sessions.length,
        credits: credits,
        streak: streak,
        lastActive: today,
        level: levelNames[levelIdx],
        publishedAt: Date.now(),
      };

      localStorage.setItem('fl_user_' + uid, JSON.stringify(statsObj));
      window.dispatchEvent(new CustomEvent('fl_stats_published', {detail: statsObj}));
    } catch(e) {
      console.warn('[FL] Stats publish error:', e);
    }
  }, 500);
}
